...
class HybridCloud(PointCloud):
    ...
    def __init__(self,
        ...
        Args:
            ...
            verts2nodes: Dict structure that maps mesh vertices to skeleton nodes. Keys are skeleton node indices,
        ...
        self._verts2nodes = None
        if verts2nodes is not None:
            self._verts2nodes = verts2nodes
    ...
    def verts2nodes(self) -> dict:
        ...
        """
        if self._verts2nodes is None:
            tree = cKDTree(self.nodes)
            dist, ind = tree.query(self.vertices, k=1)

            self._verts2nodes = {ix: [] for ix in range(len(self.nodes))}
            for vertex_idx, skel_idx in enumerate(ind):
                self._verts2nodes[skel_idx].append(vertex_idx)
        return self._verts2nodes
