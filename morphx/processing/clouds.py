...
def pca_dists(pc: PointCloud) -> np.ndarray:
    """ Calculates distances between all points in the given PointCloud and the line which is defined by
        the first principle axis.

    Args:
        pc: PointCloud with points for which distances should get computed.

    Returns:
        distances as np.ndarray
    """
    pca = PCA(n_components=1)
    pca.fit(pc.vertices)
    p1 = pca.components_[0]
    p2 = pca.mean_
    return (np.linalg.norm(np.cross(p2-p1, p1-pc.vertices), axis=1)/np.linalg.norm(p2-p1)).reshape(-1, 1)
